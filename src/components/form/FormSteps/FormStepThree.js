...
function FormStepThree({ switchActivate }) {
        ...
        {addons.map((addon) => (
          ...
          >
            <div className="checkbox">
              {addon.selected && <img src={checkboxTick} alt="checkbox tick" />}
            </div>
            <div className="addon-name">
              <div className="addon-title">{addon.title}</div>
              <div className="addon-subtitle">{addon.subtitle}</div>
            </div>
            <div className="addon-price">{`${addon.price}${
              switchActivate ? "0/yr" : "/mo"
            }`}</div>
          </div>
        ))}
      </div>
